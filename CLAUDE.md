...
**Adding a new language:** use the `/add-language` skill.
...
**Adding a new error:** use the `/add-error` skill.
...
8. **Verify changes match CI before committing** - Always run `/ci-check` (build + lint + test must all pass) before finishing.
